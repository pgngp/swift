...
{
	lookupTable3Create(refFile, seedLen, maxHitsPerQry, tupleIgnoreThreshold);
...
{
	static int numQryTuples, i, j, hash, startIdx, endIdx;
	...
	for (i = 0; i < numQryTuples; ++i)
	{
		...
		{
			tmpHitList = hitListCreateNode(_refIdx[j], _refPos[j] - i, _refPos[j]);
